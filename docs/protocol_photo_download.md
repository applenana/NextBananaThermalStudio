...
图片下载协议用于通过串口命令从设备读取一帧完整数据文件。上位机先发 `check` 命令获取文件列表，再发 `download <filename>` 逐张下载原始字节。文件格式随固件版本演进，上位机解码器兼容以下三种格式。
...
| 命令 | 方向 | 含义 |
|---|---|---|
| `check\n` | 上位机 → 设备 | 查询片上图片列表，设备返回 JSON |
| `download <filename>\n` | 上位机 → 设备 | 下载指定文件，设备返回 hex-dump 格式字节流 |

### check 响应格式（JSON）

设备返回一段 JSON，包含 `photos` 数组：

```json
{
  "photos": [
    { "name": "IMG_001.bin", "size": 3080, "mode": "full", "dataFormat": "v1full", "sn": "ABCD1234" },
    { "name": "IMG_002.bin", "size": 3092 }
  ]
}
```

字段 `mode`、`dataFormat`、`sn` 为可选；上位机据此选择 v1Simple / v1Full 解码路径。

### download 响应格式（hex-dump）

设备以类 `hexdump -C` 格式逐行输出，每行格式：

```
XXXXXXXX: XX XX XX XX ... |ASCII|
```

上位机逐行解析十六进制列，跳过地址列和 ASCII 列，拼接出原始字节。文件末尾有 `END FILE DATA` 标志行，表示传输完成。
...
上位机                                   设备
  ...
  |<─── { "photos": [...] } ───────────────|
  |                                         |
  |──── download IMG_001.bin\n ────────────>|
  |<─── 00000000: 41 20 ... |A ...|  (逐行) |
  |<─── 00000010: ...                       |
  |<─── END FILE DATA ─────────────────────|
```

> **注意**：`check` 和 `download` 操作期间上位机会暂停推流心跳，操作完成后自动恢复。禁止在 `_photoMode` 未释放时发送新的 `check` / `download`。
